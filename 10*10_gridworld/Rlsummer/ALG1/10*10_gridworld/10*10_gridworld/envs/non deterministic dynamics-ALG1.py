...
def Q_learning(epsilon1, epsilon2):
    ...
    for episodes in range(0, no_episodes):
        ...
        env.reset()
        try:
            log_results = math.log2(episodes)
            if log_results.is_integer() or episodes == 999:
                Qtable_list.append(q_table)
                print(episodes)
        except:
            print(episodes)
...
if __name__ == "__main__":
    ...
    fig1.savefig('MaxQ-expert10M-0.1.png', bbox="tight", dpi=200)
    fig1.show()
    fig2.savefig('variance-expert10M-0.1.png', bbox="tight", dpi=200)
    fig2.show()
