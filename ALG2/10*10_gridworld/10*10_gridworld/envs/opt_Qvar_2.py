...
def Q_learning(epsilon1, epsilon2, learning_rate, discount_rate, no_episodes, alpha):
    ...
    for episodes in range(0, no_episodes):
        ...
        while True:
            ...
            Var.update_secondmoment((state, action, next_state, np.argmax(q_table[next_state, :]), reward), max_value,
                                    q_visits[state, action])
...
def optimise(q_table, variance_table):
    bounds, step = list(np.linspace(1800, 0, num=1500, endpoint=False, retstep=True))
    return_list = []
    ...
    rect1 = ax1.bar(bounds, return_list, alpha=0.6, color='green')
    # rect2 = ax2.bar(bounds, wasted_episodes_list, alpha = 0.6, color = 'red')
    ...
    for rect in rect1:
        ...
        if height == max:
            ax1.annotate('{}'.format(height),
                         ...
                         textcoords="offset points",
                         ha='center', va='bottom')
    fig1.savefig("optimisation curve-2")
    # fig2.savefig("episodes in traps")
    fig1.show()
    # fig2.show()
...
if __name__ == "__main__":
    ...
    samples = 10
    ###############################
    q_table_tensor = torch.zeros((samples, 100, 4))
    variance_table_tensor = torch.zeros((samples, 100, 4))
    fig1, ax1 = plt.subplots(1, 2, figsize=(20, 10))
    fig2, ax2 = plt.subplots(1, 2, figsize=(20, 10))
    for k in range(samples):
        for i, p in enumerate(epsilon1, 0):
            for j, q in enumerate(epsilon2, 0):
                ...
                variance_table_tensor[k] = torch.from_numpy(Var.Variance)
    torch.save(q_table_tensor, "Qtable_0.1_150.pt")
    torch.save(variance_table_tensor, "variance_table_0.1_150.pt")
