...
egreedy_final = -90
egreedy_decay = 500
def calculate_expert_reward(expert_call_number):

    epsilon3 =  egreedy - (0.001*expert_call_number)
    if epsilon3< -50:
        epsilon3 = -50
    #print(epsilon3)
    return epsilon3
...
def Q_learning(epsilon1, epsilon2, learning_rate, discount_rate, no_episodes):
    epsilon1 = epsilon1
    epsilon2 = epsilon2
    Qtable_list = []
    expert_call_list = []
    expert_call_number = 0
    ...
    for episodes in range(0, no_episodes):
        ...
        try:
            ...
            if log_results.is_integer() or episodes == 9999999:
                ...
                expert_call_list.append(expert_call_number)
                print(episodes)
        except:
            print(episodes)
        ...
        while True:
            ...
            if chosen_action == 4:
                ...
                expert_call = 1
                expert_call_number += 1
                expert_reward = calculate_expert_reward(expert_call_number)
            ...
            list_next_state = env.step(action, epsilon2, expert_call, expert_reward)
    ...
    return q_table, Qtable_list, expert_call_list
...
def heatmap_display(maxQ_array, maxA_array, var_array, obs_lst, trp_lst):
    ...
    fig1.savefig('MaxQ-ALG1-10M-test.png', bbox="tight", dpi=200)
    fig1.show()
    fig2.savefig('variance-ALG1-10M-test.png', bbox="tight", dpi=200)
    fig2.show()
...
def returns_graph(Qtable_list):
        ...
        for p in range(0, 8):
            ...
            for j in start_points:
                ...
                while True:
                    ...
                    list_next_state = env.step(action, epsilon2[0], expert_call, expert_reward)
    ...
    ax.set_xlabel("number of episodes of training ( in log scale to the base 2)")
    autolabel(fig1,ax)
    plt.show()
    #plt.savefig("reward_Qlearning-ALG1-number calls")
    print(expert_calls)
...
if __name__ == "__main__":
    ...
    for i, p in enumerate(epsilon1, 0):
        for j, q in enumerate(epsilon2, 0):
            ...
            q_table, Qtable_list, expert_calls = Q_learning(p, q, learning_rate, discount_rate, no_episodes)
            print(expert_calls)
            ...
            heatmap_display(maxQ_array, maxA_array, var_array, obs_lst, trp_lst)
